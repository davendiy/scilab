...
function y_chisla=chisla_Ferma(n,a)
    y2_chisla=a^(n-1)-1
...
rand_matrix=rand_matrix*10^3
rand_matrix=round(rand_matrix)
disp("<rand_matrix>: completed")

disp(rand_matrix)
pause

p_primes=primes(10^4)
disp("<primes>:completed")   //інформація для юзера
size_primes=size(p_primes)
N=1
while p_primes(N)<10^3     //заміна всіх простих чисел<10^10 нулями
    p_primes(N)=0
